...
import logoSvg from "../assets/image/swiggy.svg";
import { PiCaretDownBold } from "react-icons/pi";
import Sidebar from "./Sidebar";
import AuthSideBar  from "./AuthBar"
...
const Header = () => {
  ...
  const address = val ? val?.address : "Unknown Location";
    ...
    <>
      <header className="fixed top-0 z-[1000] w-full h-[80px] bg-white px-5 shadow-[0_15px_40px_-20px_rgba(40,44,63,0.15)]">
        <div className="h-full min-w-[1200px] max-w-[1200px] flex mx-auto">
          <Link to="/" className="w-14 h-14 flex items-center self-center logo">
            <svg
              class="_8pSp-"
              viewBox="0 0 559 825"
              height="49"
          ...
          {(homeMatch || restroMatch) && (
            <div
              className="ml-8 h-10 flex items-center max-w-xs self-center cursor-pointer hover:text-[#FF5200] text-sm group"
              onClick={() => setShowSidebar(!showSidebar)}
            >
              <span className="relative after:content-[''] after:bottom-[-5px] after:left-0 after:absolute after:h-0.5 after:w-full after:group-hover:bg-[#FF5200] after:bg-[#3d4152]">
                <span className="font-bold">Other</span>
              </span>
              <span className="ml-2 font-light group-hover:text-[#686b78a7] text-[#686b78] truncate text-ellipsis">
                {address}
              </span>
              <span className="text-[#FF5200] text-xl ml-2">
          ...
          <ul className="flex flex-1 justify-end items-center h-full text-sm">
            <li
              className={` mr-[60px] font-semibold ${
                searchMatch && "text-[#FF5200]"
              } hover:text-[#FF5200] group`}
            >
              <Link className="flex" to="/search">
                <span className="mr-2.5 ">
                  <svg
                    className={`fill-[#3d4152] ${
                      searchMatch && "fill-[#FF5200]"
                    } group-hover:fill-[#FF5200]`}
                    viewBox="5 -1 12 25"
                    height="17"
                    width="17"
                    fill="#686b78"
            ...
            <li
              className={`mr-[60px] font-semibold hover:text-[#FF5200] group`}
            >
              {user ? <div className="flex cursor-pointer "  onClick={() => setAuthBar(!AuthBar)} >
                <span className="mr-2.5">
                  <svg
                    className="fill-[#3d4152] group-hover:fill-[#FF5200]"
                    viewBox="6 0 12 24"
                    height="19"
                    width="18"
                    fill="#686b78"
                ...
                <span >{user?.name}</span>
              </div> : <div className="flex cursor-pointer "  onClick={() => setAuthBar(!AuthBar)} >
                <span className="mr-2.5">
                  <svg
                    className="fill-[#3d4152] group-hover:fill-[#FF5200]"
                    viewBox="6 0 12 24"
            ...
            <li
              className={`last:mr-0 font-semibold ${
                checkoutMatch && "text-[#FF5200]"
              } hover:text-[#FF5200] group`}
            >
              <Link className="flex" to="/checkout">
                <span className="mr-2.5 flex relative">
                  <svg
                    className={`fill-[#60b246] group-hover:fill-[#FF5200] ${
                      checkoutMatch && "fill-[#FF5200] stroke-[#FF5200]"
                    } group-hover:stroke-[#FF5200] stroke-[#60b246]`}
                    viewBox="-1 0 37 32"
                    height="20"
                    width="22"
                    fill="#686b78"
      ...
      <Sidebar show={showSidebar} setShowSidebar={setAuthBar}  onClose={() => setShowSidebar(false)} />

      {showSidebar && <Overlay onClose={() => setShowSidebar(false)} />}

      <AuthSideBar show={AuthBar} setShowSidebar={setAuthBar} onClose={() => setAuthBar(false)} />

{AuthBar && <Overlay onClose={() => setAuthBar(false)} />}
    </>
  );
};
